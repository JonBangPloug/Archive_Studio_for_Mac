...
def run_correction(
    ...
    *,
    ...
    if preset.task_type != TASK_CORRECT:
        raise ValueError(f"Preset {preset.name!r} is not a correction preset")

    definition = get_task_definition(TASK_CORRECT)
    ...
    if not target_pages:
        raise ValueError("No pages matched the correction request")
    ...
    try:
        ...
        for batch in chunked(target_pages, batch_size):
            ...
            _emit_task_progress(
                ...
                message="Sending correction request",
            )
            ...
            for request in requests:
                ...
                if result is None:
                    message = f"Provider returned no correction result for page {request.page_sequence}"
                    errors.append(message)
                    log.error(message)
                    pages_failed += 1
                    _record_failed_checkpoint(
                ...
                content = _normalize_response_text(result.corrected_text, preset.response_prefix)
        ...
        try:
            ...
            log.warning("Could not write correction task artifact %s", task_run_id, exc_info=True)
...
def _render_prompt(page: _CorrectionTargetPage, preset: TaskPreset) -> PromptMessages:
...
def _write_completed_checkpoint(
    ...
    *,
    ...
    try:
        ...
        return
    if path is not None:
        log.info("Wrote correction checkpoint for page %s to %s", page_sequence, path)
...
def _load_target_pages(
    ...
    *,
    ...
    with project.session() as session:
        ...
        selected: list[_CorrectionTargetPage] = []
        for page in pages:
            current_original = get_current_text_version(
                session,
                page_id=page.id,
            ...
            if skip_existing and get_current_text_version(
                ...
                continue
            selected.append(
                _CorrectionTargetPage(
                    id=page.id,
                    sequence=page.sequence,
                    image_path=page.image_path,
                    source_type=page.source_type,
                    source_text=current_original.content,
                    source_text_version_id=current_original.id,
                )
            )
        return selected
